<template>
  <div>
    ...
    <div class="content-body">
      ...
      <div class="row">
        <div id="leave-grid" style="width: 100%; height: 500px" class="realgrid-type-style"></div>
      </div>

      <load-leave-modal ref="LoadLeaveModal" />
      <update-leave-modal ref="UpdateLeaveModal" />
      <add-leave-modal ref="AddLeaveModal" />
    </div>
  </div>
</template>
...
import LoadLeaveModal from "./LoadLeaveModal";
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { header: '휴직 구분', fieldName: 'A'},
        { header: {
            text: "휴직 시작일",
            template: `<p>휴직 시작일 </br><SMALL>GMT +08:00</SMALL></p>`
          }, fieldName: 'START_DATE' },
        { header: {
            text: "휴직 종료일",
            template: `<p>휴직 종료일 </br><SMALL>GMT +08:00</SMALL></p>`
          }, fieldName: 'END_DATE' },
        { header: {
            text: "급여 변동 시작일",
            template: `<p>급여 변동 시작일 </br><SMALL>GMT +08:00</SMALL></p>`
          }, fieldName: 'B' },
        { header: {
            text: "급여 변동 종료일",
            template: `<p>급여 변동 종료일 </br><SMALL>GMT +08:00</SMALL></p>`
          }, fieldName: 'C' },
      ...
      rowAttrs: ['EMP_NAM', 'HRDEPT_NAM']
    }
  },
  methods: {
    download: function() {
      this.downloadRealGridExcel( '휴직 기간 연계');
    },
    openModal: function(urlstr) {
      if(urlstr == 'LoadModal'){
        this.$refs.LoadLeaveModal.show();
      }else if(urlstr == 'AddModal'){
        this.$refs.AddLeaveModal.show();
      }
    },
    isStatus: function (){
  ...
  mounted() {
    try {
      this.createRealGrid({'domId': 'leave-grid'});
      this.asyncData();
      this.getloadData();
