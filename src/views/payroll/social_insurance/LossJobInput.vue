...
<div>
    ...
    <div class="content-body">
            ...
            <div class="col-3">
                <table-form :colgroup="['auto']">
                    <template v-slot:body>
                        <tr>
                            <table-form-label label="내부관리번호" :has-error="errors.SI_DEREGIST_MATTER_NO != ''" :required="formData.SI_DEREGIST_MATTER_NO.isRequired"/>
                        </tr>
                        <tr>
                            <table-form-item :error-msg="errors.SI_DEREGIST_MATTER_NO">
                                <ui-input :value="formData.SI_DEREGIST_MATTER_NO.value"
                                          @change="formData.SI_DEREGIST_MATTER_NO.value=$event;"
                                          :options="{readonly: formData.SI_DEREGIST_MATTER_NO.isReadOnly}"/>
                            </table-form-item>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <table-form-item>
                                <ui-input :value="formData.SI_DEREGIST_MATTER_NAME.value" @change="formData.SI_DEREGIST_MATTER_NAME.value=$event;" />
                            </table-form-item>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <table-form-item>
                                <ui-input-date :value="formData.SI_DEREGIST_MATTER_DATE.value" @change="formData.SI_DEREGIST_MATTER_DATE.value=$event;" />
                            </table-form-item>
                        </tr>
                    </template>
                </table-form>
        ...
        <button-panel
                ...
                v-on:insert="doSave"
        />
        <loss-job-person-input-modal ref="lossJobPersonInputModal" @addPersonList="addPersonList"/>
    </div>
</div>
</template>
<script>
...
import LossJobPersonInputModal from '@/components/payroll/social_insurance/LossJobPersonInputModal'
...
export default {
    ...
    data() {
        return {
            ...
            formData: {
                ...
                SI_DEREGIST_MATTER_NAME: {
                    value: '',
                },
                SI_DEREGIST_MATTER_DATE: {
                    value: ''
                },
                OPEN_TYPE: {
                    value: ''
    ...
    methods: {
        ...
        async doSave() {
            ...
            try {
                await this.$httpPost({
                    url: '/payroll/insurance/deregist/save',
                    param: {
        ...
        getGridDataList: function() {
              ...
              list.forEach((data) => {
                  returnList.push({
                      ...
                      RETIRE_YEAR: data.RETIRE_DATE.substring(0, 4),
        ...
        async openAcquisitionJobPersonInput() {
            ...
            this.$refs.lossJobPersonInputModal.show({SI_DEREGIST_MATTER_NO: this.formData.SI_DEREGIST_MATTER_NO.value});
        },

        //TODO list-> find로 수정하기
        async checkExistMatterNo() {
            try {
                let {data} = await this.$httpGet( '/payroll/insurance/deregist/matter/list', {});
                if( data.length > 0) {
                    return true;
                }
        ...
        async loadGridData() {
            ...
            if(this.payrollUtil.isEmpty(param.SI_DEREGIST_MATTER_NO)) {
                return;
            }

            try {
                const {data} = await this.$httpGet('/payroll/insurance/deregist/common-inf/in-list', param)
        ...
        async setUpdateLayout() {
            this.formData.SI_DEREGIST_MATTER_NO.value = this.$route.params.SI_DEREGIST_MATTER_NO;
            this.formData.SI_DEREGIST_MATTER_NO.isReadOnly = true;
    ...
    mounted() {
        ...
        if(! this.payrollUtil.isEmpty(this.$route.params.SI_DEREGIST_MATTER_NO)) {
            this.setUpdateLayout();
        } else {
            this.formData.SI_DEREGIST_MATTER_NO.isReadOnly = false;
        }
    },
}
</script>
