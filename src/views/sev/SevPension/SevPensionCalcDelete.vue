  ...
  <div>
    ...
    <div class="content-body">
      <div>
          ...
          <div class="form-row">
            <button-panel btnType='top'>
              <template v-slot:start>
                <button class="btn btn-md mr-5" @click="sevPensionCalcDelete()">
        ...
        <div>
          <div id="sev-pension-calc-delete-grid" style="width: 100%; height: 400px" class="realgrid-type-style"/>
        </div>
      </div>
...
export default {
      ...
      try {
        me.confirmDelete(function () {
          me.$httpPost({
            ...
            url: '/payroll/retirement/sev-pension/delete-calc'
          });
    ...
    async loadGridData() {
      ...
      try {
        let {data} = await me.$httpPost({
          param: { SEV_DATE: me.SEV_PENSION_MATTER_DATA.SEV_DATE, SEV_PENSION_SEQ: me.SEV_PENSION_MATTER_DATA.SEV_PENSION_SEQ, DELETE_LIST: 'Y', RETIRE_TYPE: 'P'},
  ...
  mounted() {
    ...
    me.setData();
    me.createRealGrid({
      domId: 'sev-pension-calc-delete-grid',
      'checkbar': 'multi',
      editable: false
    });
    me.loadGridData();
