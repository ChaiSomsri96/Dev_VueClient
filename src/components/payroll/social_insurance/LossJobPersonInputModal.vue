<template>
  <base-modal title="대상자 관리" id="loss-job-person-input-modal" :scroll="false" width="1300">
      <template v-slot:body>
        <border-box width="200" v-slot="slotProps">
            <border-box-item title="퇴사일" :width="slotProps.width" multi>
        ...
        <div>
            <div id="loss-job-person-input-modal-grid" class="realgrid-type-style" style="width: 100%; height: 300px"></div>
        </div>
      </template>
...
export default {
    name: 'loss-job-person-input-modal',
    mixins: [modal, grid],
    components: {
        BaseModal,
        BorderBox,
    ...
    methods: {
        ...
        createDynamicComponent() {
            this.createRealGrid({
                'domId': 'loss-job-person-input-modal-grid',
                'autoLoad': false,
                'checkbar': 'multi'
            });
        },
        ...
        async loadGridData() {
            try {
                ...
                const {data} = await this.$httpGet('/payroll/insurance/deregist/common-inf/not-in-list', param);
