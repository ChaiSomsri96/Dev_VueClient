        ...
        <template v-slot:body>
            <table-form :colgroup="['30%', 'auto', '15%', 'auto']">
                <template v-slot:body>
                    ...
                    <tr>
                        <table-form-label label="상실일" :has-error="errors.EMP_DEREGIST_DATE != ''" :required="formData.EMP_DEREGIST_DATE.isRequired"/>
                        <table-form-item :error-msg="errors.EMP_DEREGIST_DATE">
                            <ui-input-date :value="formData.EMP_DEREGIST_DATE.value" @change="formData.EMP_DEREGIST_DATE.value=$event"/>
                        </table-form-item>
                    ...
                    <tr>
                        <table-form-label label="상실사유" :has-error="errors.EMP_DEREGIST_CODE != ''" :required="formData.EMP_DEREGIST_CODE.isRequired"/>
                        <table-form-item :error-msg="errors.EMP_DEREGIST_CODE">
                            <ui-dropdown :items="formData.EMP_DEREGIST_CODE.items"
                                         :value="formData.EMP_DEREGIST_CODE.value"
                                         @change="formData.EMP_DEREGIST_CODE.value=$event.value;"
                                         :options="{
                                             valueField  : 'EMP_DEREGIST_CODE',
                                             labelField  : 'EMP_DEREGIST_CODE_NAME',
                                             tooltipField: 'EMP_DEREGIST_CODE_NAME'}"/>
                        </table-form-item>
                        <table-form-label label="구체적 상실사유" />
                        <table-form-item>
                            <ui-input :value="formData.EMP_DEREGIST_REASON.value"
                                      @change="formData.EMP_DEREGIST_REASON.value=$event;"/>
                        </table-form-item>
                    </tr>
                    <tr>
                        <table-form-label label="당해년도 보수총액" :has-error="errors.EMP_CURR_YR_INCOME != ''" :required="formData.EMP_CURR_YR_INCOME.isRequired"/>
                        <table-form-item :error-msg="errors.EMP_CURR_YR_INCOME">
                            <ui-input-number :value="formData.EMP_CURR_YR_INCOME.value" @change="formData.EMP_CURR_YR_INCOME.value=$event"/>
                        </table-form-item>
                        <table-form-label label="전년도 보수총액" :has-error="errors.EMP_PREV_YR_INCOME != ''" :required="formData.EMP_PREV_YR_INCOME.isRequired"/>
                        <table-form-item :error-msg="errors.EMP_PREV_YR_INCOME">
                            <ui-input-number :value="formData.EMP_PREV_YR_INCOME.value" @change="formData.EMP_PREV_YR_INCOME.value=$event"/>
                        </table-form-item>
                    </tr>
                </template>
            </table-form>
...
export default {
    ...
    methods: {
        async createDynamicComponent() {
            // 상실사유
            this.formData.EMP_DEREGIST_CODE.items = await this.getDropdownData({
                url: '/payroll/insurance/code-list',
                params: {
                    FIELD: 'EMP_DEREGIST_CODE'
                }
            });
        },
        ...
        async doSave() {
            ...
            const me = this;
            this.$httpPost({
                url: '/payroll/insurance/deregist/emp/update',
